...
def logo(n):
	return str(f"""
   \033[1;92m   _____ _    _ _    ___      ______  
   \033[1;92m  / ____| |  | | |  | \ \    / / __ \ 
   \033[1;92m | (___ | |__| | |  | |\ \  / / |  | |
   \033[1;92m  \___ \|  __  | |  | | \ \/ /| |  | |
   \033[1;92m  ____) | |  | | |__| |  \  / | |__| |
   \033[1;92|_____/|_|  |_|\____/    \/   \____/ 
   \033[1;96mOnek Din Pore Back Aslam tai free dicchi XAYAN AHMED 🔥💗
                 Stetus- Trail{H}•{K}•{M}•""")   
def logo2():
	return str(f"""
   \033[1;92   _____ _    _ _    ___      ______  
   \033[1;92  / ____| |  | | |  | \ \    / / __ \ 
   \033[1;92 | (___ | |__| | |  | |\ \  / / |  | |
   \033[1;92  \___ \|  __  | |  | | \ \/ /| |  | |
   \033[1;92  ____) | |  | | |__| |  \  / | |__| |
...
def cek_opsi_cp():
	...
	except:sys.exit(f" [{M}>{P}] tidak ada XAYAN💔 cp")
	for x in ok:
		nom.append(x)
		no+=1
...
def cek_akun():
	...
	except:sys.exit(f" [{M}>{P}] tidak ada XAYAN💔 ok")
	for x in ok:
		nom.append(x)
		no+=1
...
def cek_HASAN():
	no,nom = 0,[]
	one = input(f' [{hh}1{P}] cek XAYAN💚 akun ok\n [{hh}2{P}] cek HASAN💔 akun cp\n menu : ')
	if one in ['1','01']:
		try:ok = os.listdir('OK')
		except:sys.exit(f" [{M}>{P}] tidak XAYAN💚 ok")
		for x in ok:
			nom.append(x)
			no+=1
			try:jum= open('OK/'+x,'r').readlines()
		...
		except:sys.exit(f" [{M}>{P}] file tidak ada XAYAN💚 ok")
		print(hh+buka+P)
	elif one in ['2','02']:
		try:ok = os.listdir('CP')
		except:sys.exit(f" [{M}>{P}] tidak XAYAN💔 cp")
		for x in ok:
			nom.append(x)
			no+=1
			try:jum= open('CP/'+x,'r').readlines()
		...
		except:sys.exit(f" [{M}>{P}] file tidak ada XAYAN💔 cp")
		print(kk+buka+P)
	else:sys.exit(f" [{M}>{P}] isi yang benar")
...
def cari_nama(link):
	...
	try:
		link = r.find('a',string='Lihat XAYAN💔 Selanjutnya').get('href')
		if(link):
			print('\r sedang dump %s id'%(len(dump)),end=" ")
			sys.stdout.flush()
			cari_nama(link)
